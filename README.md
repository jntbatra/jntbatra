...
The paper I reproduced prices with a perfect-foresight estimator that only gives a divergent upper bound (56% high at 21 exercise dates). I reimplemented it with Longstaff-Schwartz, the method the paper cites but doesn't use, validated it to 1e-5 against Black-Scholes, and got it running 53× faster than serial (10× over 28-thread OpenMP).
`C++` · `CUDA` · `OpenMP`
